      ...
      <div className="w-full md:w-80 h-56 md:h-auto relative overflow-hidden bg-slate-100">
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 -translate-y-1/2 p-1.5 bg-white/80 hover:bg-white text-slate-900 rounded-full shadow opacity-0 group-hover/card:opacity-100 transition-all"
            >
              <ChevronLeft size={20} />
            </button>
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-white/80 hover:bg-white text-slate-900 rounded-full shadow opacity-0 group-hover/card:opacity-100 transition-all"
            >
            ...
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
              {images.map((_: any, idx: number) => (
                <div
                  key={idx}
                  className={`w-1.5 h-1.5 rounded-full transition-all ${currentImg === idx ? "bg-white w-4" : "bg-white/50"}`}
                />
              ))}
            </div>
          </>
        ...
        <div>
          <div className="flex justify-between items-start mb-4">
            <div>
              ...
              <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                ...
                <span className="flex items-center gap-1.5">
                  <Layers size={16} className="text-slate-400" /> Tầng{" "}
                  {cabin.deck || "2"}
                </span>
              </div>
            </div>
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-2 mb-6">
            {amenities.slice(0, 4).map((amt: any) => (
              <div
                key={amt.id}
                className="flex items-center gap-1.5 text-xs font-medium text-slate-600 bg-slate-50 px-2 py-1 rounded-md border border-slate-100"
              >
