...
export function BookingDetailPage() {
  ...
  const fetchBookingDetail = async () => {
    try {
      ...
      const res = await axios.get(`http://localhost:8081/api/bookings/${id}`, {
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center text-[#0A192F] bg-[#F8F9FA]">
  ...
  const cabin =
    booking?.details?.[0]?.cabinClass || booking?.details?.[0]?.cabin_class;
    ...
    <div className="min-h-screen bg-[#F8F9FA] py-12">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <button
          ...
          className="flex items-center gap-2 text-[#0A192F]/60 hover:text-[#0A192F] mb-6 transition-colors font-bold"
        >
        ...
        <motion.div
          ...
          className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-[#D4AF37]/30"
        >
          <div
            className={`p-8 text-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${isPaid ? "bg-[#0A192F]" : isCancelled ? "bg-[#0A192F]/60" : "bg-[#D4AF37] text-[#0A192F]"}`}
          >
            <div>
              <h1 className="text-3xl font-serif font-bold mb-2">
                {isCompleted
                  ...
                  : isPaid
                    ? "Vé Điện Tử VIP"
                    : isHolding
                      ? "Đơn Đặt Chỗ OceanaLux"
                      : "Đơn Hàng Đã Hủy"}
              </h1>
              <p className="flex items-center gap-2 opacity-90">
                Mã đặt chỗ:{" "}
                <span
                  className={`font-mono font-bold px-3 py-1 rounded tracking-widest ${isPaid ? "bg-white/20" : isCancelled ? "bg-white/20" : "bg-[#0A192F] text-white"}`}
                >
            ...
            <div
              className={`flex items-center gap-2 px-5 py-2 rounded-full font-bold shadow-sm ${isPaid ? "bg-[#D4AF37] text-[#0A192F]" : isHolding ? "bg-[#0A192F] text-white" : "bg-white/90 text-[#0A192F]/70"}`}
            >
              {isCompleted && (
                <>
                  <CheckCircle2 className="w-5 h-5" /> Hoàn Thành
                </>
              )}
              {isPaid && (
                <>
              ...
              {isHolding && (
                <>
                  <Clock className="w-5 h-5 animate-pulse" /> Chờ Thanh Toán
                </>
              )}
              {isCancelled && (
                <>
          ...
          <div className="flex flex-col md:flex-row">
            <div className="flex-1 p-8 md:border-r-2 border-dashed border-gray-200 space-y-8">
              <section>
                <h3 className="text-sm font-bold text-[#D4AF37] uppercase tracking-wider mb-4 border-b border-gray-100 pb-2">
                  Hải Trình
                </h3>
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h2 className="text-xl font-bold font-serif text-[#0A192F]">
                      {cruise?.name || "Du thuyền 5 Sao"}
                    </h2>
                    <div className="flex items-center gap-2 text-gray-500 mt-1 text-sm">
                      <MapPin className="w-4 h-4" /> Vịnh Hạ Long
                    </div>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mt-6">
                  <div className="bg-[#F8F9FA] p-4 rounded-xl border border-gray-100">
                    <div className="flex items-center gap-2 text-gray-500 mb-1 text-sm">
                      <Calendar className="w-4 h-4 text-[#D4AF37]" /> Khởi hành
                    </div>
                  ...
                  <div className="bg-[#F8F9FA] p-4 rounded-xl border border-gray-100">
                    <div className="flex items-center gap-2 text-gray-500 mb-1 text-sm">
                      <Users className="w-4 h-4 text-[#D4AF37]" /> Hạng phòng
                    </div>
                    <div className="font-bold text-[#0A192F]">
                      {cabin?.name || "Standard"}
                    </div>
            ...
            <div className="w-full md:w-80 p-8 bg-[#F8F9FA] flex flex-col items-center justify-center text-center relative">
              ...
              <div
                className={`bg-white p-4 rounded-2xl shadow-md border border-[#D4AF37]/20 mb-6 ${!isPaid ? "opacity-20 grayscale blur-[2px]" : ""}`}
              >
                <QRCodeSVG
                  value={`OCEANALUX-${booking.booking_code}-${booking.id}`}
                  size={160}
                  level="H"
                  includeMargin={true}
                  fgColor="#0A192F"
              ...
              {!isCompleted && !isPaid && !isCancelled && (
                <p className="text-sm text-[#0A192F] font-bold mb-4 bg-[#D4AF37]/20 py-2 px-4 rounded-lg">
                  Mã QR khả dụng sau thanh toán
                </p>
              )}
              {isCompleted && (
                <p className="text-sm text-[#0A192F] font-bold mb-4 bg-[#D4AF37]/20 py-2 px-4 rounded-lg">
                  Cảm ơn bạn đã đồng hành cùng OceanaLux!
                </p>
              )}
              {isCancelled && (
                <p className="text-sm text-[#0A192F]/60 font-bold mb-4 bg-gray-200 py-2 px-4 rounded-lg">
                  Đơn hàng không còn hiệu lực
                </p>
              )}

              <div className="w-full mb-6">
                <div className="text-gray-500 text-sm mb-1 uppercase tracking-wider font-bold">
                  Tổng Thanh Toán
                </div>
                <div
                  className={`text-3xl font-bold ${isCancelled ? "text-gray-400 line-through" : "text-[#D4AF37]"}`}
                >
                ...
                {isHolding && (
                  <>
                    <button
                      ...
                      className="w-full bg-[#D4AF37] text-[#0A192F] py-3 rounded-xl font-bold hover:brightness-110 transition-all shadow-md"
                    >
                      Thanh Toán Ngay
                    </button>
                    <button
                      ...
                      className="w-full bg-transparent text-[#0A192F]/60 border-2 border-[#0A192F]/20 py-3 rounded-xl font-bold hover:bg-[#0A192F]/5 hover:text-[#0A192F] transition-colors"
                    >
