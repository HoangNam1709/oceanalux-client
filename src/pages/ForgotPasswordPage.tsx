...
import { Ship, Mail, ArrowLeft, Lock, Eye, EyeOff, Loader2, ShieldCheck, Timer } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#F8F9FA] flex items-center justify-center py-12 px-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-serif font-bold text-[#0A192F] mb-2">Phục Hồi Mật Khẩu</h1>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-100 p-8">
          ...
          {step === 1 && (
            <form onSubmit={handleSendOtp} className="space-y-6">
              ...
              >
                {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Đang kiểm tra...</> : "Nhận Mã Xác Thực"}
              </button>

              <Link to="/login" className="flex items-center justify-center gap-2 text-sm text-slate-500 hover:text-[#0A192F] font-medium transition-colors mt-4">
          ...
          {step === 2 && (
            <form onSubmit={handleResetPassword} className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div>
                <label className="block text-sm font-bold text-slate-700 mb-2 text-center">Mã OTP (6 chữ số)</label>
              ...
              <div className="space-y-4 pt-4 border-t border-slate-100">
                <div>
                  ...
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amber-500">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-12 pr-12 py-3 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#0A192F] transition-all"
                    />
                    <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-amber-500">
              ...
              >
                {loading ? <><Loader2 className="w-5 h-5 animate-spin" /> Đang cập nhật...</> : "Xác nhận Đổi Mật Khẩu"}
              </button>

              <div className="flex items-center justify-between pt-4">
                <button type="button" onClick={() => setStep(1)} className="text-sm text-slate-500 hover:text-[#0A192F] font-medium flex items-center gap-1">
                ...
                {countdown > 0 ? (
                  ...
                  <button type="button" onClick={() => handleSendOtp()} disabled={loading} className="text-sm text-[#D4AF37] hover:text-amber-600 font-bold">
                    Gửi lại mã OTP
                  </button>
                )}
              </div>
