...
import {
  Search, Calendar, Users, MapPin, ChevronRight, Star,
  Clock, PawPrint, Baby, Anchor, Award, Shield,
  ArrowRight, Play, Quote, Sparkles, Compass, Utensils, Waves, Wind
} from "lucide-react";
...
const HERO_SLIDES = [
  ...
  {
    image: "https://images.unsplash.com/photo-1576723860479-4459fdf0bc48?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxjcnVpc2UlMjBzaGlwJTIwaW5maW5pdHklMjBwb29sJTIwb2NlYW4lMjBkZWNrJTIwbHV4dXJ5fGVufDF8fHx8MTc3NTE1MzkyNHww&ixlib=rb-4.1.0&q=80&w=1080",
...
const EXPERIENCES = [
  {
    icon: Utensils, title: "Ẩm Thực Michelin",
    ...
    image: "https://images.unsplash.com/photo-1674168461837-9e2be6a263da?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxsdXh1cnklMjBjcnVpc2UlMjBzaGlwJTIwZmluZSUyMGRpbmluZyUyMHJlc3RhdXJhbnR8ZW58MXx8fHwxNzc1MTUzOTI0fDA&ixlib=rb-4.1.0&q=80&w=1080",
  },
  {
    icon: Sparkles, title: "Spa & Phục Hồi",
...
const TESTIMONIALS = [
  {
    name: "Nguyễn Minh Anh", role: "CEO, TechVN", rating: 5, avatar: "M",
  ...
  {
    name: "Trần Phương Linh", role: "Giám đốc Marketing", rating: 5, avatar: "L",
  ...
  {
    name: "Lê Quốc Hùng", role: "Luật sư", rating: 5, avatar: "H",
...
function StatCard({ value, label }: { value: string; label: string }) {
  ...
  return (
    <div ref={ref} className="text-center group">
      <div className="font-serif text-[clamp(2.2rem,4vw,3.5rem)] leading-none text-[#D4AF37] group-hover:scale-110 transition-transform duration-500">{count}</div>
      <div className="text-slate-300 text-sm mt-3 uppercase tracking-[0.2em]">{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="w-full bg-[#F8F9FA] font-sans overflow-x-hidden selection:bg-[#D4AF37] selection:text-[#0A192F]">
     ...
     {/* ═══════════════════════════════════════════════════════
      ...
      ═══════════════════════════════════════════════════════ */}
      <section ref={heroRef} className="relative w-full overflow-hidden h-[80svh] min-h-[600px] max-h-[850px]">
        {/* Slideshow */}
        <AnimatePresence mode="sync">
          <motion.div key={activeSlide} initial={{ opacity: 0, scale: 1.06 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.4 }} className="absolute inset-0" style={{ y: heroY }}>
            <img src={HERO_SLIDES[activeSlide].image} alt="Luxury Cruise" className="w-full h-full object-cover" />
          </motion.div>
        </AnimatePresence>
        ...
        <motion.div style={{ opacity: heroOpacity }} className="absolute inset-0 flex flex-col justify-center px-6 md:px-20 max-w-7xl mx-auto left-0 right-0 z-10 pb-20">
          <AnimatePresence mode="wait">
            <motion.div key={activeSlide} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.8 }} className="max-w-3xl">
              <div className="flex items-center gap-3 mb-6">
                <span className="w-10 h-px bg-[#D4AF37]" />
                <span className="text-xs font-bold uppercase tracking-[0.35em] text-[#D4AF37]">{HERO_SLIDES[activeSlide].tagline}</span>
              </div>
              <h1 className="text-white font-serif mb-6 text-[clamp(2.8rem,7vw,6.5rem)] leading-[1.05] tracking-tight">
                {HERO_SLIDES[activeSlide].heading.split('\n').map((line, i) => (
                  <span key={i} className="block">{i === 1 ? <span className="text-[#D4AF37] drop-shadow-lg italic font-medium">{line}</span> : line}</span>
                ))}
              </h1>
              <p className="text-slate-200 mb-12 max-w-xl text-lg leading-relaxed font-light">{HERO_SLIDES[activeSlide].sub}</p>
            </motion.div>
          </AnimatePresence>
        </motion.div>
        ...
        <div className="absolute bottom-8 left-0 right-0 max-w-5xl mx-auto px-4 z-20">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.8 }}
            ...
            <form onSubmit={handleSearch} className="flex flex-col md:flex-row w-full items-center divide-y md:divide-y-0 md:divide-x divide-slate-100">
              ...
              <div className="flex-1 w-full px-4 py-3 hover:bg-slate-50/70 rounded-xl transition-colors group cursor-pointer flex items-center gap-3">
                ...
                <div className="flex flex-col w-full">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 group-hover:text-[#D4AF37] transition-colors mb-0.5 cursor-pointer">Điểm đến</label>
              ...
              <div className="w-full md:w-auto p-2 md:p-0 md:pl-3">
                <button type="submit" className="w-full md:w-auto bg-gradient-to-br from-[#D4AF37] to-[#e8c84a] text-[#0A192F] hover:shadow-[0_8px_20px_rgba(212,175,55,0.4)] rounded-xl px-7 h-12 md:h-[3.25rem] font-bold uppercase tracking-wider text-xs transition-all duration-300 flex items-center justify-center hover:scale-105">
...
{/* ═══════════════════════════════════════════════════════
      ...
      <section className="py-28 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <p className="text-[#D4AF37] uppercase tracking-[0.2em] text-sm font-bold">Bộ Sưu Tập Giới Hạn</p>
            <span className="w-8 h-px bg-[#D4AF37]" />
          </div>
        ...
        {trendingCruises.length === 0 ? (
          <div className="text-center py-20 text-slate-500 font-medium animate-pulse flex flex-col items-center gap-3">
            <Anchor className="w-8 h-8 text-[#D4AF37] animate-spin" style={{ animationDuration: '3s' }} />
      ...
      <section className="py-28 overflow-hidden bg-[#0A192F] relative">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-4">
              <span className="w-8 h-px bg-[#D4AF37]" />
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#D4AF37]">Trải nghiệm độc quyền</span>
              <span className="w-8 h-px bg-[#D4AF37]" />
            </div>
            <h2 className="font-serif text-white text-[clamp(2.5rem,4vw,3.5rem)]">Cuộc Sống Trên Boong Tàu</h2>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-stretch">
            <div className="lg:w-1/3 flex flex-row lg:flex-col gap-4 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0 snap-x">
              {EXPERIENCES.map((exp, i) => {
                ...
                return (
                  <button key={exp.title} onClick={() => setActiveExp(i)} className={`flex-1 min-w-[280px] lg:min-w-0 lg:flex-none flex items-start gap-5 p-6 rounded-2xl text-left transition-all duration-300 snap-center border ${active ? 'bg-white/5 border-[#D4AF37]/40 shadow-[0_0_30px_rgba(212,175,55,0.1)]' : 'bg-transparent border-white/5 hover:bg-white/5'}`}>
                    <div className={`p-3 rounded-xl shrink-0 transition-colors ${active ? 'bg-[#D4AF37]' : 'bg-[#D4AF37]/10'}`}>
                      <Icon className={`w-6 h-6 ${active ? 'text-[#0A192F]' : 'text-[#D4AF37]'}`} />
                    </div>
                    <div>
                      <div className={`font-bold text-lg mb-2 transition-colors ${active ? 'text-white' : 'text-slate-400'}`}>{exp.title}</div>
                      {active && <motion.p initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="text-sm text-slate-300 leading-relaxed">{exp.desc}</motion.p>}
                    </div>
                  </button>
                );
              })}
            ...
            <div className="lg:w-2/3 relative rounded-3xl overflow-hidden min-h-[450px] shadow-2xl border border-white/10">
              <AnimatePresence mode="wait">
                <motion.img key={activeExp} src={EXPERIENCES[activeExp].image} alt={EXPERIENCES[activeExp].title} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: 0.6 }} className="w-full h-full object-cover absolute inset-0" />
              </AnimatePresence>
              <div className="absolute inset-0 bg-gradient-to-t from-[#0A192F]/90 via-[#0A192F]/20 to-transparent" />
              <div className="absolute bottom-8 left-8 right-8">
                <h3 className="font-serif text-white text-3xl mb-3">{EXPERIENCES[activeExp].title}</h3>
                <p className="text-slate-200 text-sm leading-relaxed max-w-xl">{EXPERIENCES[activeExp].desc}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="relative">
              ...
              <img src="/images/Aboutus_images_home.jpg" alt="Heritage" className="relative z-10 w-full rounded-2xl object-cover shadow-2xl aspect-[4/5] grayscale hover:grayscale-0 transition-all duration-700" />
              <div className="absolute -bottom-6 -right-6 md:-bottom-10 md:-right-10 bg-[#0A192F] text-white p-8 z-20 shadow-2xl rounded-2xl max-w-xs border border-[#D4AF37]/30">
                ...
                <p className="text-sm uppercase tracking-widest font-light text-slate-300">Năm Kiến Tạo <br/>Kỳ Nghỉ Hoàn Mỹ</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="flex flex-col">
              <div className="flex items-center gap-3 mb-5">
                <span className="w-8 h-px bg-[#D4AF37]" />
                <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#D4AF37]">Di Sản & Định Hướng</span>
              </div>
              <h2 className="text-[clamp(2.2rem,3.5vw,3.2rem)] font-serif font-bold text-[#0A192F] mb-8 leading-tight">
                VỀ CHÚNG TÔI & <br/><span className="text-[#D4AF37]">CAM KẾT CỦA CHÚNG TÔI</span>
              </h2>
              <div className="text-lg text-slate-600 font-light mb-12 leading-relaxed">
                <p>Hơn 20 năm kiên trì theo đuổi sự hoàn hảo, chúng tôi mang đến những kỳ nghỉ 5 sao cá nhân hóa, nơi mỗi chi tiết đều được chăm chút tỉ mỉ. Không chỉ là một chuyến đi, chúng tôi kiến tạo một trải nghiệm sống đỉnh cao trên đại dương.</p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {[
                  { icon: Calendar, title: "Chính sách Đặt phòng", desc: "Đơn giản & nhanh chóng." },
                  { icon: Clock, title: "Chính sách Hủy chuyến", desc: "Linh hoạt cho mọi hành trình." },
                  { icon: PawPrint, title: "Chính sách Thú cưng", desc: "Chấp nhận thú cưng nhỏ." },
                  { icon: Baby, title: "Chính sách Trẻ em", desc: "Ưu đãi cho gia đình & trẻ em." },
                ].map((p, i) => (
                  <div key={p.title} className="flex items-start gap-4 p-5 bg-[#F8F9FA] border border-slate-100 rounded-xl hover:border-[#D4AF37]/30 hover:shadow-lg transition-all group">
                    ...
                    <div>
                      <h4 className="font-bold text-[#0A192F] mb-1 text-sm">{p.title}</h4>
      ...
      <section className="py-28 px-4 sm:px-6 bg-[#0A192F] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="text-center mb-16">
            <div className="flex items-center justify-center gap-3 mb-4">
              <span className="w-8 h-px bg-[#D4AF37]" />
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#D4AF37]">Cảm nhận từ khách hàng</span>
              <span className="w-8 h-px bg-[#D4AF37]" />
            </div>
            <h2 className="font-serif text-white text-[clamp(2.5rem,4vw,3.5rem)]">Những Câu Chuyện Thật</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, i) => (
              <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15, duration: 0.7 }}
                ...
                <p className="text-slate-200 leading-relaxed mb-8 flex-1 text-[15px] italic">"{t.text}"</p>
                <div className="flex items-center gap-1.5 mb-6">
                  {[...Array(t.rating)].map((_,j) => <Star key={j} className="w-4 h-4 fill-[#D4AF37] text-[#D4AF37]" />)}
                </div>
                <div className="flex items-center gap-4 pt-6 border-t border-white/10">
                  <div className="w-12 h-12 rounded-full flex items-center justify-center bg-[#D4AF37] text-[#0A192F] font-bold text-lg shrink-0 shadow-[0_0_15px_rgba(212,175,55,0.4)]">{t.avatar}</div>
                  <div>
      ...
      <section className="py-24 px-4 sm:px-6 max-w-5xl mx-auto -translate-y-12 relative z-20">
        <div className="relative rounded-3xl overflow-hidden shadow-2xl">
          <img src="./images/Banner.avif" alt="Newsletter" className="absolute inset-0 w-full h-full object-cover" />
          <div className="absolute inset-0 bg-[#0A192F]/85 backdrop-blur-sm" />
          
          <div className="relative p-12 md:p-16 text-center">
            <div className="flex items-center justify-center gap-3 mb-6">
              <Sparkles className="w-4 h-4 text-[#D4AF37]" />
              <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#D4AF37]">Ưu đãi độc quyền</span>
              <Sparkles className="w-4 h-4 text-[#D4AF37]" />
            </div>
            <h2 className="font-serif text-white mb-6 text-[clamp(2.2rem,4vw,3rem)] leading-tight">Nhận Ưu Đãi Trước Mọi Người</h2>
            <p className="text-slate-300 mb-10 max-w-xl mx-auto text-lg">Đăng ký nhận bản tin để không bỏ lỡ các chương trình khuyến mãi sớm và hành trình mới.</p>
